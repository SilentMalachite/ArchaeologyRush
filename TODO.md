...
- [ ] 発見確率テーブルの詳細値を、確定済みのユーザー仕様に沿って反映する
- [ ] 重要遺物判定や評価ロジックの仮置き部分を、ユーザー仕様に沿って再確認する
- [ ] 道具耐久や層混入の扱いで、現在は UI や終了判定に十分反映していない箇所を洗い出して実装へ反映する

### P2: 品質ゲートと運用整備

- [ ] `mix quality` に `mix credo` と `mix dialyzer` をどう組み込むか決め、日常的に回せる品質ゲートへ揃える
- [ ] Elixir Desktop の配布ターゲットとビルド手順を決める
- [ ] セーブデータ形式と移行方針を決める

## ユーザー仕様の確認が必要な項目

- [x] 最終レポートの必須入力項目
- [ ] 発見確率テーブルの詳細条件 (層、座標、ターン、補正要素)
- [ ] 重要遺物の定義と評価点
- [ ] 道具耐久が 0 になった後の扱い
